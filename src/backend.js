...
export class Doctor {
  ...
  async getListAmount(condition, location) {
    try {
      ...
      console.log("Website" + jsonifiedResponseList.data[0].practices[0].website);
      console.log( "Accepting New Patients " + jsonifiedResponseList.data[0].practices[0].accepts_new_patients);
      console.log("Phone Number " + jsonifiedResponseList.data[0].practices[0].phones[0].number);
      console.log("Address " + jsonifiedResponseList.data[0].practices[0].visit_address.street + " " + jsonifiedResponseList.data[0].practices[0].visit_address.city + " " +  jsonifiedResponseList.data[0].practices[0].visit_address.state + " " + jsonifiedResponseList.data[0].practices[0].visit_address.zip);
      return listamount;
    } catch(error) {
      console.error("There was an error handling your request: " + error.message);
    }
